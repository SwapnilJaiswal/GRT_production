...
/**
 * Servlet implementation class CloneHandler
 */
@WebServlet("/CloneHandler")
public class CloneHandler extends HttpServlet {
...
public String AllowedToEdit = "false";
	 ...
	 */
		...
		try {
			...
			System.out.println("Old Sno for cloning is " +SNo );
			ResultSet rs = s.executeQuery("select * from at.dbo.ProjectDetails_Competency2 where SNo='"+SNo+"'");
			System.out.println("select * from at.dbo.ProjectDetails_Competency2 where SNo='"+SNo+"'");
			...
			AllowedToEdit="false";
		RequestDispatcher rd = request.getRequestDispatcher("competency/CloneRequest.jsp?SNo="+SNo);
			    ...
			    throws SQLException
			  {
		        ...
		        if(rsmd.getColumnType(i)==java.sql.Types.ARRAY){
		         ...
		         obj.put(column_name, rs.getInt(column_name));
		        	...
		        	String sDate = null;
		        	String currentDtStr = null;
					if(column_name.equalsIgnoreCase("CreatedDate")){
						DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
			            java.util.Date currentDate = new java.util.Date();
			            currentDtStr = dateFormat.format(currentDate);
							        		
					}
		        	if(date!=null)
		        	{
		        		if(column_name.equalsIgnoreCase("CreatedDate")){
		        			sDate = currentDtStr; // While cloning, set current date for created date
		        		}
		        		else{
		        			sDate = date.toString();
			        		String[] tokens = sDate.split("-");
